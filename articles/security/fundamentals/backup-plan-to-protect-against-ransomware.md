...
Ransomware attacks deliberately encrypt or erase data and systems to force your organization to pay money to cybercriminals. These attacks target your data, your backups, and also key documentation required for you to recover without paying the threat actors (as a means to increase the chances your organization will pay).
...
Ransomware is a type of extortion attack that encrypts files and folders, preventing access to important data and systems. Threat actors use ransomware demand money from targets, usually in the form of cryptocurrencies, in exchange for a decryption key or in exchange for not releasing sensitive data to the dark web or the public internet.

While early ransomware mostly used malware that spread with phishing or between devices, human-operated ransomware has emerged where a gang of active threat actors, driven by human attack operators, target all systems in an organization (rather than a single device or set of devices). An attack can:

- Encrypt your data
- Exfiltrate your data
- Corrupt your backups

The ransomware leverages the threat actors’ knowledge of common system and security misconfigurations and vulnerabilities to infiltrate the organization, navigate the enterprise network, and adapt to the environment and its weaknesses as they go.

Ransomware can be staged to exfiltrate your data first, over several weeks or months, before the ransomware actually deploys on a specific date.
...
The best way to prevent falling victim to ransomware is to implement preventive measures and have tools that protect your organization from every step that threat actors take to infiltrate your systems.
...
Always be prepared for the event that a ransomware attack impacts your organization. One of the most important steps you can take to protect your data and avoid paying a ransom is to have a reliable backup and restore plan for your business-critical information. Since ransomware threat actors have invested heavily into neutralizing backup applications and operating system features like volume shadow copy, it is critical to have backups that are inaccessible to a malicious threat actor.
...
The backup data is stored in Azure storage and the guest or threat actor has no direct access to backup storage or its contents. With virtual machine backup, the backup snapshot creation and storage is done by Azure fabric where the guest or threat actor has no involvement other than quiescing the workload for application consistent backups. With SQL and SAP HANA, the backup extension gets temporary access to write to specific blobs. In this way, even in a compromised environment, existing backups can't be tampered with or deleted by the threat actor.
...
As mentioned earlier, you should assume that at some point in time, you will be impacted by a ransomware attack. Identifying your business-critical systems and applying best practices before an attack will get you back up and running as quickly as possible. 
...
| Protect (or print) supporting documents and systems required for recovery such as restoration procedure documents, CMDB, network diagrams, and SolarWinds instances. | Threat actors deliberately target these resources to impact your ability to recover. |
| Ensure you have well-documented procedures for engaging any third-party support, particularly support from threat intelligence providers, antimalware solution providers, and from the malware analysis provider. Protect (or print) these procedures. | Third-party contacts might be useful if the given ransomware variant has known weaknesses or decryption tools are available. |
...
| Task | Detail |
| --- | --- |
| Early in the attack, engage third-party support, particularly support from threat intelligence providers, antimalware solution providers and from the malware analysis provider. | These contacts might be useful if the given ransomware variant has a known weakness or decryption tools are available. <br><br>[Microsoft Incident Response team (formerly DART/CRSP)](https://www.microsoft.com/security/blog/2019/03/25/dart-the-microsoft-cybersecurity-team-we-hope-you-never-meet/) can help protect you from attacks. Microsoft Incident Response engages with customers around the world, helping to protect and harden against attacks before they occur, as well as investigating and remediating when an attack has occurred. <br><br>Microsoft also provides Rapid Ransomware Recovery services. Services are exclusively delivered by the Microsoft Global [Compromise Recovery Security Practice (CRSP)](https://www.microsoft.com/security/blog/2021/06/09/crsp-the-emergency-team-fighting-cyber-attacks-beside-customers/). The focus of this team during a ransomware attack is to restore authentication service and limit the impact of ransomware. <br><br>Microsoft Incident Response and CRSP are part of Microsoft’s [Industry Solutions Delivery](https://www.microsoft.com/en-us/msservices/security) security service line. |
...
| Use a safety scanner and other tools for full operating system restore as well as data restore scenarios. | [Microsoft Safety Scanner](/windows/security/threat-protection/intelligence/safety-scanner-download) is a scan tool designed to find and remove malware from Windows devices. Simply download it and run a scan to find malware and try to reverse changes made by identified threats. |
...
3. Investigate and remediate the original breach (engage the [Microsoft Incident Response](https://www.microsoft.com/security/blog/2019/03/25/dart-the-microsoft-cybersecurity-team-we-hope-you-never-meet/) to help)
